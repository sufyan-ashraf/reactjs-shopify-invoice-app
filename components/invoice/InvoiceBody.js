...
export default function InvoiceBody({ invoice }) {
    ...
    return (
        <Wrapper id="invoice-div" exit={{ opacity: 0, transition: { duration: 1 }}}>
            <Title>
                <Id><span>#</span>{invoice.order_number}</Id>
                <Description>{invoice.line_items[0].origin_location.name}</Description>
            </Title>

            <SenderAddress>
                <span>{invoice.line_items[0].origin_location.address1}</span>
                <span>{invoice.line_items[0].origin_location.city}</span>
                <span>{invoice.line_items[0].origin_location.zip}</span>
                <span>{invoice.line_items[0].origin_location.country_code}</span>
            </SenderAddress>

            <Dates>
                <Date>
                    <DateTitle>Invoice Date</DateTitle>
                    <Subheading as="span">{dayjs(invoice.created_at).format('DD MMM YYYY')}</Subheading>
                </Date>
                <Date>
                    <DateTitle>Payment Due</DateTitle>
                    <Subheading as="span">N/A</Subheading>
            ...
            <InvoiceTable className="invoice-table" items={invoice.line_items} total={invoice.current_total_price}/>
        </Wrapper>
    )
}